...
\autor{ Douglas Geovanini de Paiva Mosca Leite, \\ Luiz Otávio Andrade Soares \\ \& Marcos Carvalho Ferreira}
\local{Brasil}
\data{2025, Lavras}
\instituicao{%
...
\preambulo{Trabalho Prático Final -- GCC118  Programação Matemática -- PCC540  Linear and Integer Programming.}
% ---
...
Ao longo das iterações, a temperatura é progressivamente reduzida por meio de um fator de resfriamento (\( \alpha \in (0,1)\)), seguindo a atualização:
...
Foi implementada uma estratégia de adaptação dinâmica da perturbação, na qual o parâmetro $strength$ que define a intensidade da perturbação, é ajustado dinamicamente durante a execução do algoritmo. Inicialmente, $strength$ é definido com o valor padrão. Sempre que o algoritmo ultrapassa um limiar pré-definido de iterações consecutivas sem melhorias na solução, o valor de $strength$ é incrementado em uma unidade, até atingir um limite máximo pré-definido. 

\subsubsection{Reinicialização}

Foi implementado uma estratégia de reinicialização para evitar cenários de exaustão da região de busca local. Dessa forma, quando o algoritmo executa um número pré-definido de iterações consecutivas sem que o \textit{Simulated Anealing} aceite nenhuma nova solução o procedimento é acionado:
...
\item Define $strength = valor\;inicial + 1$ (ligeiramente maior que o inicial)
...
Este capítulo descreve a metodologia empregada para a avaliação do Solver Gurobi e a implementação algoritmo ILS proposto para o ALWABP descrito no capítulo \ref{cap:ils}. São detalhados os procedimentos de otimização de parâmetros, a seleção das instâncias de teste, o protocolo experimental adotado e as métricas utilizadas para análise de desempenho.
...
\item Limite de tempo de 700 segundos por trial
